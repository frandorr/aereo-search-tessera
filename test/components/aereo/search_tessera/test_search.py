...
def test_search_empty_region(plugin: SearchTessera, mock_registry_file: Path) -> None:
    """Verify search in a region with no tiles returns empty GeoDataFrame with correct schema."""
    intersects = box(10.0, 10.0, 10.1, 10.1)
    searcher = SearchTessera(intersects=intersects, registry_path=mock_registry_file)
    result = searcher()
...
def test_search_schema_validation(plugin: SearchTessera, mock_registry_file: Path) -> None:
...
def test_search_temporal_filter(plugin: SearchTessera, mock_registry_file: Path) -> None:
...
def test_search_spatial_filter(plugin: SearchTessera, mock_registry_file: Path) -> None:
